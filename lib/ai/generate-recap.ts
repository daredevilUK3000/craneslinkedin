...
- You are recapping REAL community responses, not generating new content. Only reference the highlighted responses given to you below — never invent a quote, a name, or a stance that wasn't provided.
- Quote or closely paraphrase 2-3 of the given highlighted responses, crediting the display name if one is given, or referring to them anonymously ("one rigger", "another commenter") if none is given.
...
export async function generateRecap(params: {
  ...
  const highlightsText = params.highlights
    .map((h, i) => {
      ...
      return `${i + 1}. ${who}${stance} — source: ${h.source}\n"${text}"`;
    })
    .join('\n\n');
